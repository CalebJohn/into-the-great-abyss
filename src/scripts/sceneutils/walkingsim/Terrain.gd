...
var noise = load("res://src/scripts/sceneutils/walkingsim/noise.gd").new()

func height(pos):
	return -5.0+5.0*pow(noise.fbmsimplex(pos),3)+5.0*noise.cliffNoise(pos*0.05)

func _ready():
	groundMat = FixedMaterial.new()
	groundMat.set_parameter(FixedMaterial.PARAM_DIFFUSE, Color(65/255.0, 120/255.0, 52/255.0))
	#groundMat.set_light_shader(FixedMaterial.LIGHT_SHADER_TOON)
	#groundMat.set_parameter(FixedMaterial.PARAM_SHADE_PARAM, 0.5)
